#!/bin/bash
if [[ $(id -u) -ne 0 ]]; then
  echo "Hata: Bu script sudo ile çalıştırılmalıdır. Örnek: sudo ./backup-postgres-test.sh"
  exit 1
fi
...
BACKUP_NAME="${PROJECT_TITLE}_test_$(date +%Y-%m-%d_%H-%M-%S).dump"
CONTAINER_PATH="/tmp/${BACKUP_NAME}"

mkdir -p "${BACKUP_TEST_DIR}"

echo "Konteyner: ${CONTAINER_NAME}"
echo "Yedek dizini: ${BACKUP_TEST_DIR}"
...
docker cp "${CONTAINER_NAME}:${CONTAINER_PATH}" "${BACKUP_TEST_DIR}/"
...
echo "Dosya: ${BACKUP_TEST_DIR}/${BACKUP_NAME}"
echo ""
echo "Dizin içeriği:"
ls -la "${BACKUP_TEST_DIR}" 2>/dev/null || dir "${BACKUP_TEST_DIR}" 2>/dev/null || echo "(liste alınamadı - dizini elle kontrol edin)"
